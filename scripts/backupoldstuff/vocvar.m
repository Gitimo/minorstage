...
Voc_1689=[0.6,0.61,0.62,0.63,0.64,0.65];
Voc_1688=1.01;
for m=1:6
	Q_led_1689(m)=J_rad_1689*exp(q*Voc_1689(m)/(k*Troom))/J_sc_1689;
	lum_1689(:,m)=exp(q*Voc_1689(m)/(k*Troom))*bb(dlam,F_bb,Troom).*EQElong_1689(:,2);
end
Q_led_1688=J_rad_1688*exp(q*Voc_1688/(k*Troom))/J_sc_1688
Voc_lim_1689=k*Troom/q * log(J_sc_1689(1) / J_rad_1689 +1)
Voc_lim_1688=k*Troom/q * log(J_sc_1688 / J_rad_1688 +1)

lum_1689=exp(q*Voc_1689(1)/(k*Troom))*bb(dlam,F_bb,Troom).*EQElong_1689(:,2);
lum_1688=exp(q*Voc_1688/(k*Troom))*bb(dlam,F_bb,Troom).*EQElong_1688(:,2);
plot(dlam*10^(9),lum_1689(:,1),"b",dlam*10^(9),lum_1688,"g"); #plotting in nm
axis([600,1000,0,1.01]);
title "Calculated spectral luminescence. Blue is W1689, green W1688"
xlabel "wavelength (nm)";
ylabel "luminescence in A/m^2/nm";
print -dpng lumines.png
disp("press return for next plot");
pause();
