...
lAlGaAs(m)=10*indAlGaAs(1,m)/wave(m);
lGaAs(m)=10*indGaAs(1,m)/wave(m);

rAg(m)=abs(multidiel([1,nAlGaAs(m),nAg(m)],[lAlGaAs(m)],1))^2;
rAu(m)=abs(multidiel([1,nAlGaAs(m),nAu(m)],[lAlGaAs(m)],1))^2;
end
